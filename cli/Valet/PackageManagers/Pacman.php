...
class Pacman implements PackageManager
{
    ...
    {
        output('<info>[' . $package . '] is not installed, installing it now via Yay...</info> 🍻');

        output('<info>Running: yay --noconfirm --needed -S' . $package . '</info>');
        $this->cli->run(trim('yay --noconfirm --needed -S ' . $package), function ($exitCode, $errorOutput) use ($package) {
            output($errorOutput);

            throw new DomainException('Yay was unable to install [' . $package . '].');
        });
    }
